...
## 5. Browser lab bundle (what-if approximation, added 2026-09-19)

`packages/app/public/data/lab-edges.bin.gz` + `lab-meta.json` (generated by
`pipelines/export_lab_bundle.ts`, `pnpm lab`):

- **What**: the recruited subgraph for the Interactive Lab — union over all
  taste seed groups (sugar/bitter/water/lowsalt GRNs + proboscis MNs from
  `seeds.json`) of a 3-hop expansion through edges with `syn_count >= 3`,
  plus the 8 readout motor neurons. 28,620 neurons, 999,090 edges of the
  full 15.09M-edge graph (6.6% of neurons, 6.6% of edges).
- **Contents**: CSR adjacency (off/dst/w) with the REAL signed weights
  (`syn_count × NT sign × W_syn`), main-bundle index map, raw FlyWire soma
  coordinates, seed-group lists, readout indices, cell-type dictionary.
- **Use**: the browser worker runs the same LIF model (sim-core, paper
  parameters) live on this subgraph for user-driven what-if stimuli. This is
  an APPROXIMATION of the full connectome — the UI labels it as such, and the
  Validate action quantifies the gap against the precomputed full-model run
  (sugar @ 100 Hz: sub-sim MN9_r ≈ 75–90 Hz vs full-model 86.4 Hz depending
  on trial count; bitter co-activation → 0 Hz, suppression reproduced).
- **Provenance/licence**: derived entirely from the sources above
  (CC-BY-4.0), no new data.

## 6. Primary papers to cite in product/UI
